...
config =
	...
	output:
		...
		publicPath:    if hmr then 'http://localhost:8080/dist/' else '/dist/'
...
config.module.loaders = [
	...
	{
		test: /\.css$/
		loader: if hmr then 'style-loader!css-loader' else ExtractText.extract 'css-loader'
	}
	...
	# Modernizr #
	# Fix Modernizr's exported module.  Even though it has a "module.exports" it
	# doesn't let the detected properties get queried
	{ test: /modernizr\.js$/, loader: 'exports-loader?window.Modernizr'}
...
config.plugins = [
	...
	new webpack.BannerPlugin "📝 Placehold.Club 💾 #{moment().format('M.D.YY')} 👍"
